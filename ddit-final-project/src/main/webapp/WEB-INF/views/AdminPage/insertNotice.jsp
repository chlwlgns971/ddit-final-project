...
<!DOCTYPE html>
<form:form modelAttribute="article" enctype="multipart/fomr-data" action="${pageContext.request.contextPath }/mypage/admin/insertNF"
	...
	<div class="table-dashboard">
		<div class="row">
			<div class="mb-7 mb-lg-0">
				...
				<h3>
					<input type="hidden" name="artBCode"  value="${artBCode}"/>
					<c:set var="artBCode" value="${artBCode}"/>
					<c:choose>
						<c:when test="${artBCode eq 'BC003' }">
							 공지사항 등록
						</c:when>
						<c:when test="${artBCode eq 'BC005' }">
							 FAQ 등록
						</c:when>
						<c:otherwise>
							error
