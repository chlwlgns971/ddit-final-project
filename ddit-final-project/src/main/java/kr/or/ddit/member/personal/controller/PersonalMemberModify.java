...
@Slf4j
@Controller
@RequestMapping("/mypage/member/personal/personalMemberEdit")
public class PersonalMemberModify {
	@Inject
	 ...
	 */
	@GetMapping
	public String personalMemberView(
			Model model
		...
		// jsp에 전달할 객체구하기
		PersonalMemberVO memberVO = pmService.retrievePersonalMember(memId);
		ProfileImageVO imgInfoVO = acService.retrieveImgInfo(memId);
		AccountVO acc=acService.selectAccount(memId);
		...
		return "myPage/member/personal/personalMemberModify";
	}
	 ...
	 */
	@PostMapping
	public String personalMemberModify(
			@ModelAttribute("memberVO") PersonalMemberVO personalMemberVO,
			@AuthenticationPrincipal AccountVOWrapper accountWrapper,
			@AuthenticationPrincipal(expression = "realUser") AccountVO account
			, Model model
			, HttpServletRequest request
			, String accIntro
			, @RequestPart(name = "multipartImage") MultipartFile multipartImage
			, HttpSession session
	) throws IOException {
		...
		// 인증객체에서 ID 추출
		String memId = test1.getAccId();
		// 해당 ID PersonalMemberVO 객체에 주입
		personalMemberVO.setMemId(memId);


		//이미지의 존재여부와 이미지의 정보를 가져온다.
		int rowcnt=pmService.searchImage(memId);
		
		//ProfileImageVO에서 기존의 이미지를 가져온다
		ProfileImageVO profImg=pmService.retrieveImgInfo(memId);
		...
		int profileCount = acService.countProfileImage(memId);
		...
		if(rowcnt==0) {
			...
			save = new File(request.getSession().getServletContext().getRealPath(imageFolderURL));
			// 파일 삭제를 위한 경로 설정
			getFileName = new File(request.getSession().getServletContext().getRealPath(imageFolderURL)
					+ "/" + profImg.getImgNam());			
		}
...
========================================================================  */	
		...
		if (multipartImage.getSize()==0) {
			...
			if (accNum > 0) {
				// 수정한 거 던져주고 결과값 받기
				ServiceResult result = pmService.modifyPersonalMember(personalMemberVO);
				...
				case OK:
					...
					viewName = "myPage/member/personal/personalMemberModify";
					
					//수정이 성공하고 나서의 뷰에 보내줄 객체를 찾는다.
					PersonalMemberVO memberVO = pmService.retrievePersonalMember(memId);
			
					profImg.setAccId(memId);
...
========================================================================  */		
		...
		//<<<<<<<<<<<<<<<<<<<<기존에 파일이 존재하는 경우>>>>>>>>>>>>>>
			if (profileCount > 0) {
				// DB와 기존파일을 삭제한다.
				imgDeleteNum = acService.deleteProfilImage(memId);
				getFileName.delete();
